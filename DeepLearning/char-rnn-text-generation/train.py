from torch import optim, nn, save, cuda
...
def train(model, train_ds, val_ds, train_opts, exp_dir=None):
    ...
    model.to('cuda' if cuda.is_available() else model.fc.weight.device)
...
def fit(model, data_dl, criterion, optimizer=None):
    ...
    one_hot_gen = E(model.rnn.vocab_size, device=device)
